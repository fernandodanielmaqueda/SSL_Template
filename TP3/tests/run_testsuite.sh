...
#   Caso 1: Es la ruta a un archivo regular
...
#   Caso 2: Es el nombre de un comando definido en la variable de entorno PATH
...
for input_path in "${this_dir}"/input/test_*"${input_extension}"; do
    ...
    if ! [ -f "${actual_output_path}" ]; then
		...
		output_line_count=$(awk 'END {print NR}' "${this_dir}/output/${test_name}_clean.txt")
		expected_line_count=$(awk 'END {print NR}' "${this_dir}/output/expected/${test_name}_clean.txt")
